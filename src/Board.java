...
public class Board {
	...
	public Board() {
		 ...
		 * { board[row][col] = new Tile(null, false, false); } }
		 * 
		 * getTile(7, 0).setPiece(new Rook(1, new Position(7, 0)));
		 * getTile(7,4).setPiece(new King(0, new Position(7,4))); getTile(6,
		 * 3).setPiece(new Rook(0, new Position(6, 3)));
		 */
	...
	public boolean isLegalMove(Position fromPos, Position toPos, Tile[][] aBoard) {
		...
		if (pieceToMove.getName().equals("Pawn")) {
			if (!isWithinPawnRangeOfMovement(pieceToMove, toPos, aBoard)) {
				System.out.println("Illegal: Not within Pawn ROM");
				return false;
			}
		} else {
		...
		for (int row = 0; row < SIZE; row++) {
			for (int col = 0; col < SIZE; col++) {
				...
				if (board[row][col].hasPiece()) {
					Piece piece = Piece.createPiece(board[row][col].getPiece(), board[row][col].getPiece().hasMoved());
					copy[row][col].setPiece(piece);
				} else {
					Piece piece = Piece.createPiece(board[row][col].getPiece(), false);
					copy[row][col].setPiece(piece);
				}
	...
	public boolean isWithinPawnRangeOfMovement(Piece pawn, Position toPos, Tile[][] aBoard) {
		...
		System.out.println(pawn.hasMoved());
