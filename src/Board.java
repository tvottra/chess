...
public class Board {
    ...
    public boolean isLegalMove(Position fromPos, Position toPos, Tile[][] aBoard) {
        ...
        if (pieceToMove.getName().equals("Pawn")) {
            ...
            if (!isWithinHotSpots(pieceToMove, toPos, board) || (board[toRow][toCol].hasPiece()
                ...
                // System.out.println("Illegal: proposed move not within actual range of
                // movement!");
                // }
    ...
    public boolean castleAble(Position fromPos, Position toPos, Tile[][] aBoard) {
        Piece king = aBoard[fromPos.getRow()][fromPos.getColumn()].getPiece();
        Position toPosCheck;
        Position rookToPos;
    ...
    private Position findKingPosition(int color, Tile[][] aBoard) {
        return findKingPosition(color, aBoard, 0, 0);
    }


    private Position findKingPosition(int color, Tile[][] aBoard, int row, int col) {

        if (aBoard[row][col].hasPiece()) {
            if (row == SIZE - 1 && col == SIZE - 1) {
                if (aBoard[row][col].getPiece().getName().equals("King") && aBoard[row][col].getPiece().getColor() == color) {
                    return new Position(row, col);
                } else {
                    return null;
                }
            } else if (aBoard[row][col].getPiece().getName().equals("King") && aBoard[row][col].getPiece().getColor() == color) {
                return new Position(row, col);
            } else if (col == SIZE - 1) {
                return findKingPosition(color, aBoard, row + 1, 0);
            } else {
                return findKingPosition(color, aBoard, row, col + 1);
            }
        } else if (col == SIZE - 1) {
            return findKingPosition(color, aBoard, row + 1, 0);
        } else {
